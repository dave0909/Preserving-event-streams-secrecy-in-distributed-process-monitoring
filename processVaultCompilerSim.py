...
def main():
    ...
    build_command = "CGO_CFLAGS=-I/opt/ego/include CGO_LDFLAGS=-L/opt/ego/lib ego-go build -o main main.go"

    try:
        subprocess.run(build_command, shell=True, check=True)
    except subprocess.CalledProcessError as e:
        print("Build command failed:", e)
        sys.exit(1)

    if isInSimulation == "true":
        print("Running the Process Vault in simulation mode...")
        run_command = f"./main {event_dispatcher_address} {extraction_manifest_file_path} "
    else:
        print("Running the Process Vault in standard mode...")
        run_command = f"./main {event_dispatcher_address} {extraction_manifest_file_path} "
    ...
    try:
        ...
        #    monitor_thread = threading.Thread(target=monitor_process, args=(process.pid, log_file))
